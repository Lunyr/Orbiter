...
const log = logger.getLogger('NotBiddableRange');

export default async (job) => {
  log.debug("NotBiddableRange handler reached");

  job.progress(1);

  // Sanity check
  if (job.data.event.name !== 'NotBiddableRange')
  ...
  if (userAddr.success && userAddr.data !== null) {
    await addNotification(userAddr.data, 'NotBiddableRange', {
      bidder: user.address,
      scope: evData.scope,
