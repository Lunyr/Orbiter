 ...
 */
export default (tableName = DEFAULT_TABLE_NAME, maxAttempts = DEFAULT_MAX_ATTEMPTS) => {
  const queue = knex({
    client: 'sqlite3',
    connection: {
  ...
  const revert = async (job_id, progress, err) => {
    const job = await queue(tableName)
      .where({
        job_id: job_id,
      })
      .select();
